...
class SessionStateValidator:
    ...
    VALID_TRANSITIONS = {
        ...
        (ChatSessionState.AWAITING_CODE, ChatSessionState.AWAITING_CODE),  # retry with same customer
    ...
    @staticmethod
    def _enforce_target_state_invariants(
        ...
        if state == ChatSessionState.CODE_EXPIRED:
            if customer_id is not None:
                raise ValueError(
                    "CODE_EXPIRED state should have customer_id cleared"
                )
        ...
        if state == ChatSessionState.ANONYMOUS:
            if customer_id is not None:
                raise ValueError(
                    "ANONYMOUS state should have customer_id cleared"
                )
